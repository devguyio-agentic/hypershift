...
func testAutoscaling(ctx context.Context, mgtClient crclient.Client, hostedCluster *hyperv1.HostedCluster, numNodes, max int32) func(t *testing.T) {
	return func(t *testing.T) {
		...
		nodepool := getFirstNodePool(t, ctx, mgtClient, hostedCluster.Namespace)
...
func testAutoscalingBalancing(ctx context.Context, mgtClient crclient.Client, hostedCluster *hyperv1.HostedCluster, numNodes int32, additionalNP *hyperv1.NodePool) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// Get the newly created NodePool
		defaultNodePool := getFirstNodePool(t, ctx, mgtClient, hostedCluster.Namespace)
...
const (
	// aggressiveDelayAfterAddSeconds is the CAS delay-after-add setting used in pause regression tests.
	aggressiveDelayAfterAddSeconds int32 = 30
	// aggressiveUnneededDurationSeconds is the CAS unneeded-duration setting used in pause regression tests.
	aggressiveUnneededDurationSeconds int32 = 60
	// casScaleDownWaitDuration is the time to wait for CAS to attempt scale-down during pause tests.
	// Calculated as unneeded + delay-after-add + buffer.
	casScaleDownWaitDuration = 3 * time.Minute
)

// getFirstNodePool lists NodePools in the given namespace and asserts exactly one exists.
func getFirstNodePool(t *testing.T, ctx context.Context, mgtClient crclient.Client, namespace string) *hyperv1.NodePool {
	t.Helper()
	g := NewWithT(t)
	...
	e2eutil.EventuallyObject(t, ctx, "autoscaler deployment to have required settings and be ready",
		...
		[]e2eutil.Predicate[*appsv1.Deployment]{
			func(dep *appsv1.Deployment) (done bool, reasons string, err error) {
				...
				return util.IsDeploymentReady(ctx, dep), "autoscaler deployment not ready", nil
			},
		},
...
// 6. Wait for CAS to have time to attempt scale-down (unneededDuration)
...
func testAutoscalerRespectsNodePoolPause(ctx context.Context, mgtClient crclient.Client, hostedCluster *hyperv1.HostedCluster, numNodes, max int32) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// Ensure autoscaling is enabled on the NodePool so this test is self-contained
		// and does not depend on a prior subtest having already enabled it.
		err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		// Step 1: Configure aggressive CAS scale-down timers on the HostedCluster.
		err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
			if err := mgtClient.Get(ctx, crclient.ObjectKeyFromObject(hostedCluster), hostedCluster); err != nil {
				return err
			}
		...
		// Step 2: Generate workload to scale up to max nodes.
		memCapacity := nodes[0].Status.Allocatable[corev1.ResourceMemory]
		g.Expect(memCapacity).ShouldNot(BeNil())
		g.Expect(memCapacity.String()).ShouldNot(BeEmpty())
		bytes, ok := memCapacity.AsInt64()
		...
		// Step 3: Pause the NodePool. This propagates cluster.x-k8s.io/paused to the MachineDeployment.
		err = retry.RetryOnConflict(retry.DefaultRetry, func() error {
			if err := mgtClient.Get(ctx, crclient.ObjectKeyFromObject(nodepool), nodepool); err != nil {
				return err
			}
		...
		// Step 4: Delete workload so nodes become unneeded.
		cascadeDelete := metav1.DeletePropagationForeground
		err = guestClient.Delete(ctx, workload, &crclient.DeleteOptions{
			PropagationPolicy: &cascadeDelete,
		})
		...
		// Step 5: Wait for CAS unneeded duration + buffer to give CAS time to attempt scale-down.
		// With unneeded=60s and delayAfterAdd=30s, we wait 3 minutes total.
		// If the CAS fix is working, replicas should NOT change.
		t.Logf("Waiting %s for CAS to attempt scale-down on paused MachineDeployment...", casScaleDownWaitDuration)
		time.Sleep(casScaleDownWaitDuration)

		// Step 6: Verify MachineDeployment replicas have NOT been decremented.
		err = mgtClient.Get(ctx, crclient.ObjectKey{Namespace: controlPlaneNamespace, Name: nodepool.Name}, md)
		g.Expect(err).NotTo(HaveOccurred(), "failed to get MachineDeployment")

		replicasAfterWait := *md.Spec.Replicas
		t.Logf("MachineDeployment replicas after wait: %d (was %d before pause)", replicasAfterWait, replicasBeforePause)
		...
		// Step 8: After unpause, CAS may resume normal scale-down immediately since the
		// workload is gone and aggressive timers are satisfied. Verify the cluster converges
		// cleanly back to the autoscaling minimum.
		_ = e2eutil.WaitForNReadyNodes(t, ctx, guestClient, numNodes, hostedCluster.Spec.Platform.Type)
