...
type webhookServer struct {
	logger            *slog.Logger
	patchNodeSelector []byte // when /spec exists but nodeSelector is empty
	patchWithSpec     []byte // when /spec is nil/absent
...
func newWebhookServer(logger *slog.Logger) *webhookServer {
...
func (s *webhookServer) handleStats(w http.ResponseWriter, _ *http.Request) {
	stats := map[string]interface{}{
		...
		"total":          s.mutated.Load() + s.skipped.Load() + s.denied.Load() + s.errors.Load(),
	}
	w.Header().Set("Content-Type", "application/json")
